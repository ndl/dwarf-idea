/*
 * The code is based on https://github.com/airlift/aircompressor/blob/master/src/main/java/io/airlift/compress/zstd/FseTableReader.java
 *
...
class FseTableReader
{
    ...
    public int readFseTable(FiniteStateEntropy.Table table, ByteBuffer buffer, int inputOffset, int inputLimit, int maxSymbol, int maxTableLog)
    {
        // read table headers
        ...
        int bitStream = buffer.getInt(input);
        ...
        while (remaining > 1 && symbolNumber <= maxSymbol) {
            if (previousIsZero) {
                ...
                while ((bitStream & 0xFFFF) == 0xFFFF) {
                    ...
                    if (input < inputLimit - 5) {
                        input += 2;
                        bitStream = (buffer.getInt(input) >>> bitCount);
                    }
                    else {
                        // end of bit stream
                        bitStream >>>= 16;
            ...
            bitStream = buffer.getInt(input) >>> (bitCount & 31);
        }
        ...
        for (int i = 0; i < tableSize; i++) {
            int symbol = (int)table.symbol[i] & 0xFF;
